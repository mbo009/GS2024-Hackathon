...
def add_new_book():
    ...
    try:
        ...
        return Response(
            error_message_helper("Please provide a proper JSON body."),
            400,
    ...
    if "error" in resp:
        ...
        book = Book.query.filter_by(
            user=user, book_title=request_data.get("book_title")
        ).first()
        if book:
            return Response(
                error_message_helper("Book Already exists!"),
                400,
                mimetype="application/json",
            )
        else:
            ...
            return Response(
                json.dumps(responseObject), 200, mimetype="application/json"
            )
...
def get_by_title(book_title):
    ...
    if "error" in resp:
        ...
        raw_query = text(f"SELECT * FROM books WHERE book_title = '{book_title}'")
        book = db.session.execute(raw_query).fetchall()
        if book:
            responseObject = {
            ...
            return Response(
                error_message_helper("Book not found!"),
                404,
                mimetype="application/json",
            )
